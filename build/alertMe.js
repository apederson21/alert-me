...
function loadConfig() {
    ...
    fetch('/alertMe.json').then(function (response) {
...
module.exports = {
    ...
    var merged = Object.assign(base, target);
    return merged;
}
...
module.exports = {
    ...
    var type = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 'notification';
    var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
    var config = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : null;

    // they did not provide options or config, return error
    if (options === null || config === null) {
        var errName = options === null ? "Missing Options" : "Missing Config";
        var errMessage = options === null ? "Options are missing." : "Configuration missing.";
        return createError(errName, errMessage);
    }

    // if they pass an empty object, handle it
    if (Object.keys(options).length === 0) {
        var _errName = "Missing Options";
        var _errMessage = "Options are missing.";
        return createError(_errName, _errMessage);
    }

    // check to see if there are current alerts in the DOM
    // and reject new one if there are
    var currentAlerts = document.getElementsByClassName('alertMe_alert');
    ...
    var htmlOutput = document.createElement('div'); // parent div
...
function buildItem(type, options, config, htmlOutput) {
    ...
    var alertEffects = ['slideIn'];

    // declare a unique ID (time string)
    var itemId = new Date().toISOString();
    ...
    var optionClass = options.class || 'default';
    ...
    if (!!options.ok && type === 'alert' || type === 'alert') {
        // defaults
        var btnText = 'Ok';
        var btnClickEvt = 'alertMe.close(\'' + itemId + '\')';
        if (!!options.ok) {
    ...
    if (htmlOutput.innerHTML.length > 0) {
        var result = domAppend(htmlOutput);
        // check for auto close if DOM append was successful
        // and type is notification or ribbon
        if (result && type !== 'alert') {
            if (options.autoClose) {
                var duration = options.autoCloseDuration || config.defaultDuration;
        ...
        var errName = "HTML error";
        var errMessage = '"HTML could not be generated for ' + type + '."';
        return createError(errName, errMessage);
    }
}
...
var updateConfig = require('./config/update');
var dom = require('./dom');
...
getConfigFile().then(function (data) {
...
function setupFunctions() {
    global.alertMe.notification = function (options) {
        return dom.buildHTML('notification', options, configData);
    };
    global.alertMe.ribbon = function (options) {
        return dom.buildHTML('ribbon', options, configData);
    };
    global.alertMe.alert = function (options) {
        return dom.buildHTML('alert', options, configData);
    };
}

}).call(this,typeof global !== "undefined" ? global : typeof self !== "undefined" ? self : typeof window !== "undefined" ? window : {})
},{"./config/get":1,"./config/update":2,"./dom":3,"q":6}],5:[function(require,module,exports){
